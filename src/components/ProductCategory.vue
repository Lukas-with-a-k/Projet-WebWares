<template>
  ...
  <div class="product-category">
    ...
    <div class="card" v-for="(prod, index) in filteredProducts" :key="index">
      <img  :src="require(`@/assets/${prod.image}`)" :alt="prod.titre" />
      <h4>{{ prod.titre }}</h4>
      <p>Minimum Order Quantity: {{ prod.moq }}</p>
      <p v-if="isMember">{{ prod.prix }} €</p><br><br>
    </div>
    </div>
    <router-link :to="`/product-details/${products.id}`">Voir Détails</router-link>
  </div>  
</template>
